...
// Verzeichnis der aktuellen Datei relativ zum doc_root
...
// Script-URL bestimmen
$scriptUrl = strtok($_SERVER['REQUEST_URI'] ?? $_SERVER['SCRIPT_NAME'] ?? '/markdown/render.php', '?');

$htmlContent = preg_replace_callback(
    '/<a\s([^>]*?)href="([^"]*?\.md(?:#[^"]*)?)"([^>]*?)>/i',
    function ($match) use ($relativeDir, $extraQuery, $scriptUrl) {
        $before = $match[1];
        $href   = $match[2];
        $after  = $match[3];

        // Absolute URLs und Anchors nicht umschreiben
        ...
        // Pfad relativ zum doc_root zusammenbauen
        $filePath = $relativeDir ? $relativeDir . '/' . $href : $href;

        $params = ['file' => $filePath];
        $queryString = http_build_query($params) . ($extraQuery ? '&' . $extraQuery : '');

        return '<a ' . $before . 'href="' . htmlspecialchars($scriptUrl . '?' . $queryString . $anchor) . '"' . $after . '>';
    },
    $htmlContent
);
